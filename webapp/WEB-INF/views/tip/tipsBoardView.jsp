...
$(document).ready(function(){
	...
	$("#tipsBoardList").click(function(){
		var params = $.param(defaultParams);
		$(location).attr("href", "/Tip/tips?"+params);
	});
	
	$("#addReply").click(function(){
		var url = "/Tip/addTipsReply";
	...
	$("#tipsBoardDelete").click(function(){
		var url = "/Tip/delTipsBoardData";
		var params = $.extend({}, defaultParams, {brdid:"${paramVO.brdid}"});
		$.post(url, params, function(data){
			alert("게시글이 삭제되었습니다.");
			$(location).attr("href","/Tip/tips?"+$.param(defaultParams));
		});
	});
	...
	if(userno == writerno){
		$("#tipsBoardUpdate").show();
		$("#tipsBoardDelete").show();
	}
	$("#tipsBoardUpdate").click(function(){
		var url = "/Tip/tipsBoardWrite?";
	...
	$("#likes").click(function(){
		var params = $.param($.extend({},defaultParams, {brdid:"${paramVO.brdid}"}));
		$.post("/Tip/modTipsLikes", params, function(data){
			var msg = "";
			if(data == "Fail"){
				msg = "이미 추천하였습니다.";
	...
	$("#dislike").click(function(){
		var params = $.param($.extend({},defaultParams, {brdid:"${paramVO.brdid}"}));
		$.post("/Tip/modTipsDislikes", params, function(data){
			var msg = "";
			if(data == "Fail"){
				msg = "이미 비추하였습니다.";
			}else if(data == "Success"){
...
function likes(like, repid){
	...
	$.post(url, params, function(data){
		...
		if(like == 'Y'){
			...
			if(data == "Fail"){
				url = "/Tip/undoTipsBoardRepDislikes";
				$.post(url, params, function(data){
					msg = "비추천 취소하였습니다.";
					alert(msg);
					if(data == 1){
...
function modReply(repid){
	...
	var url = "/Tip/findTipsBoardReplyData";
	var params = $.extend({}, defaultParams, {repid:repid});
	$.post(url, params, function(data){
...
function comment(repid, bOpen){
	if(bOpen == 'y'){
		...
		var url = "/Tip/findTipsCommentList";
...
function updComment(prepid, repid){
	...
	$.post(url, params, function(data){	
		...
		$(location).attr("href", "/Tip/TipsBoardView?"+urlParams +"#"+prepid);
		location.reload(true);
	});
}
...
function addComment(repid){
	...
	$.post(url, params, function(data){
		var urlParams = $.param({brdid:"${paramVO.brdid}", mypage:"${paramVO.mypage}"});
		$(location).attr("href", "/Tip/TipsBoardView?"+urlParams +"#"+repid);
		location.reload(true);
	}).error(function(){
		alert();
	});
...
#tipsBoardDelete, #tipsBoardUpdate,#tipsBoardList{
	margin-left:5px; 
	line-height: 20px; 
	width:45px; 
	...
	<div class="row">
		<div class="main_body" style="overflow: hidden;">
			...
			<div class="center_menu" style="float: left; margin-left: 35px; width: 700px;">
				...
				<div id="tipsBoard_main" style="width: 100%; border: 1px solid #cacaca; margin-top: 5px; padding: 10px; background-color: white;">
					<div id="tipsBoard_title" style="width: 100%; background-color: lightgrey; font-size: 20px; padding:5px; border-top: 2px solid grey; ">
						<b>[${vo.typename}] ${vo.title }</b> <span style="float: right; font-size:14px;"> ${vo.regdate }</span><br/>
					</div>
					<div id="tipsBoard_extra" style="width:100%; background-color: white; padding:5px; font-size:12px;">
					...
					<div id="tipsBoard_content" style="width:100%; min-height: 300px; margin-top:10px;">
						${vo.content }
					</div>
					...
					<div style="float: left; width:100%;">
						<button class="btn delete" id="tipsBoardDelete" style="float: right; display:none;"><span style="font-size:80%;">삭제</span></button>
						<button class="btn confirm" id="tipsBoardList" style="float: right;"><span style="font-size:80%;">목록</span></button>			
						<button class="btn update" id="tipsBoardUpdate" style="float:right; display:none;"><span style="font-size:80%;">수정</span></button>	
					</div>
					<div style="clear:both;"></div>
					<div id="tipsBoard_reply" style="margin-top:20px; border-radius:2em; border: 1px solid #cacaca; padding: 10px; font-size: 12px;">
						댓글쓰기<br/>
