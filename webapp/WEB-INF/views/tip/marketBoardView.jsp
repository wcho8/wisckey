...
$(document).ready(function(){
	...
	$("#marketBoardDelete").click(function(){
		var url = "/Tip/delMarketBoardData";
	...
	$("#likes").click(function(){
		var params = $.param($.extend({},defaultParams, {brdid:"${paramVO.brdid}"}));
		$.post("/Tip/modMarketLikes", params, function(data){
			var msg = "";
			if(data == "Fail"){
				msg = "이미 추천하였습니다.";
	...
	$("#dislike").click(function(){
		var params = $.param($.extend({},defaultParams, {brdid:"${paramVO.brdid}"}));
		$.post("/Tip/modMarketDislikes", params, function(data){
			var msg = "";
			if(data == "Fail"){
				msg = "이미 비추하였습니다.";
			}else if(data == "Success"){
...
function likes(like, repid){
	...
	$.post(url, params, function(data){
		...
		if(like == 'Y'){
			if(data == "Fail"){
				url = "/Tip/undoMarketBoardRepLikes";
				//TODO 컨트롤러에 만들기
				$.post(url, params, function(data){
					msg = "추천 취소하였습니다.";
					alert(msg);
			...
			if(data == "Fail"){
				url = "/Tip/undoMarketBoardRepDislikes";
				$.post(url, params, function(data){
					msg = "비추천 취소하였습니다.";
					alert(msg);
					if(data == 1){
...
function delReply(repid){
	var url = "/Tip/delMarketBoardReply";
...
function modReply(repid){
	...
	var url = "/Tip/findMarketBoardReplyData";
	var params = $.extend({}, defaultParams, {repid:repid});
	$.post(url, params, function(data){
...
function confirmUpdate(repid){
	...
	var url ="/Tip/modMarketBoardReply";
...
function comment(repid, bOpen){
	if(bOpen == 'y'){
		...
		var url = "/Tip/findMarketCommentList";
...
function updComment(prepid, repid){
	...
	$.post(url, params, function(data){	
		...
		$(location).attr("href", "/Tip/MarketBoardView?"+urlParams +"#"+prepid);
		location.reload(true);
	});
}
...
function addComment(repid){
	...
	$.post(url, params, function(data){
		var urlParams = $.param({brdid:"${paramVO.brdid}", mypage:"${paramVO.mypage}"});
		$(location).attr("href", "/Tip/MarketBoardView?"+urlParams +"#"+repid);
		location.reload(true);
	}).error(function(){
		alert();
	});
